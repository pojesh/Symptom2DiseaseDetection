...
@st.cache_resource
def initialize_chromadb(_medquad_df):
    ...
    try:
        ...
        if collection.count() == 0:
            ...
            _medquad_df.dropna(subset=['question', 'answer', 'focus_area'], inplace=True)
            documents = [f"Question: {row['question']}\nFocus Area: {row['focus_area']}\n\n{row['answer']}" for index, row in _medquad_df.iterrows()]
...
def generate_explanation(query, symptoms, diseases, evidence):
    ...
    try:
        ...
        prompt = f"""
            ...
            6. Start with a clear warning that this is not a medical diagnosis and the user should consult a doctor.
...
def main():
    ...
    st.write("Describe your symptoms in the text box below and click 'Predict' to get an analysis.")

    try:
        clf, le, symptom_columns, symptom_df, medquad_df = load_models()
        collection = initialize_chromadb(medquad_df)
        model = initialize_sentence_transformer()

        user_nl = st.text_area("Describe your symptoms:", height=150)
        ...
        if st.button("Predict"):
            ...
            with st.spinner("Analyzing your symptoms..."):
                top_symptoms = get_top_symptoms(user_nl, symptom_columns, model)

                if not top_symptoms:
                    st.error("No relevant symptoms detected. Please try again with a more detailed description.")
                    return
                ...
                for d, p in zip(top3_diseases, top3_probs):
                    st.metric(label=d, value=f"{p*100:.2f}% likelihood")

                with st.spinner("Generating explanation..."):
                    evidence = retrieve_evidence(user_nl, top3_diseases[0], collection)
                    explanation = generate_explanation(user_nl, top_symptoms, top3_diseases, evidence)
